...
public class ApiConfig {
    ...
    public void loadJar(boolean useCache, String spider, LoadConfigCallback callback) {
        ...
        if (jarUrl.startsWith("assets://")) {
            // For bundled jars, avoid being blocked by stale cache. If cache load fails, recopy from assets.
            if (cache.exists() && useCache && jarLoader.load(cache.getAbsolutePath())) {
                callback.success();
                return;
            }
            if (!copyAssetJarToCache(jarUrl, cache)) {
                callback.error("");
                return;
            }
    ...
    private boolean copyAssetJarToCache(String jarUrl, File cache) {
        String assetPath = jarUrl.substring("assets://".length());
        try {
            File cacheDir = cache.getParentFile();
            if (!cacheDir.exists())
                cacheDir.mkdirs();
            if (cache.exists())
                cache.delete();
            InputStream inputStream = App.getInstance().getAssets().open(assetPath);
            FileOutputStream fos = new FileOutputStream(cache);
