...
public class VodController extends BaseController {
    ...
    protected void initView() {
        ...
        mPlayerCacheBtn = findViewById(R.id.play_cache);
    ...
    private Runnable mUpdateBottomCacheRun = new Runnable() {
        @Override
        public void run() {
            ...
            updateBottomTimeInfo(duration, position);
            mHandler.postDelayed(this, 1000);
        }
    };

    private void updateBottomTimeInfo(int duration, int position) {
        int percent = mControlWrapper == null ? 0 : mControlWrapper.getBufferedPercentage();
        if (percent < 0) {
            percent = 0;
        } else if (percent > 100) {
            percent = 100;
        }
        long totalBufferedMs = duration <= 0 ? 0 : duration * percent / 100L;
        long aheadBufferMs = Math.max(0L, totalBufferedMs - Math.max(0, position));
        mTotalTime.setText(PlayerUtils.stringForTime(duration));
        if (mPlayerCacheBtn != null) {
            mPlayerCacheBtn.setText("缓存" + PlayerUtils.stringForTime((int) aheadBufferMs));
        }
    }
    ...
    protected void onPlayStateChanged(int playState) {
        ...
        switch (playState) {
            case VideoView.STATE_IDLE:
                mHandler.removeCallbacks(mUpdateBottomCacheRun);
                break;
            case VideoView.STATE_PLAYING:
                startProgress();
                mHandler.removeCallbacks(mUpdateBottomCacheRun);
                mHandler.post(mUpdateBottomCacheRun);
                break;
