...
import com.github.tvbox.osc.util.DynamicPinyinCandidateUtil;
...
public class SearchActivity extends BaseActivity {
    ...
    protected void init() {
        // 启动时异步预热片名缓存
        DynamicPinyinCandidateUtil.warmUpCacheAsync();
        initView();
        initViewModel();
        initData();
    }
    ...
    private void initView() {
        ...
        wordAdapter.setOnItemClickListener(new BaseQuickAdapter.OnItemClickListener() {
            ...
            public void onItemClick(BaseQuickAdapter adapter, View view, int position) {
                String selected = wordAdapter.getItem(position);
                String input = etSearch.getText().toString().trim();
                if (DynamicPinyinCandidateUtil.isPinyinInitials(input)) {
                    selected = truncateCandidateByInputLength(selected, input.length());
                }
                search(selected);
            }
        });
        ...
        keyboard.setOnSearchKeyListener(new SearchKeyboard.OnSearchKeyListener() {
            ...
            public void onSearchKey(int pos, String key) {
                if (pos > 1) {
                    ...
                    if (text.length() > 0) {
                        // 检测输入是否为拼音首字母，若是则显示候选字
                        updateCandidatesForPinyin(text);
                    }
                } else if (pos == 1) {
                    String text = etSearch.getText().toString().trim();
                    if (text.length() > 0) {
                ...
                .execute(new AbsCallback<String>() {
                    ...
                    public void onSuccess(Response<String> response) {
                        try {
                            ...
                            DynamicPinyinCandidateUtil.addMovieNames(hots);
                        } catch (Throwable th) {
                            th.printStackTrace();
                        }
                    }
    ...
    private void searchData(AbsXml absXml) {
        if (absXml != null && absXml.movie != null && absXml.movie.videoList != null && absXml.movie.videoList.size() > 0) {
            ...
            List<String> namesToCache = new ArrayList<>();
            for (Movie.Video video : absXml.movie.videoList) {
                if (video != null && !TextUtils.isEmpty(video.name)) {
                    namesToCache.add(video.name);
                }
                if (matchKeyword(video.name, searchTitle)) {
                    data.add(video);
                }
            }
            DynamicPinyinCandidateUtil.addMovieNames(namesToCache);
    ...
    private boolean matchKeyword(String name, String keyword) {
        ...
        if (!isAsciiAlphaNumeric(normalizedKeyword)) {
            return false;
        }

        String fullPinyin = alphaNumericOnly(Pinyin.toPinyin(name, "").toLowerCase());
        if (fullPinyin.contains(normalizedKeyword)) {
            return true;
        }

        String initials = buildPinyinInitials(name);
        return initials.startsWith(normalizedKeyword);
    }

    private String truncateCandidateByInputLength(String candidate, int inputLength) {
        if (TextUtils.isEmpty(candidate) || inputLength <= 0) {
            return candidate;
        }
        if (candidate.length() <= inputLength) {
            return candidate;
        }
        return candidate.substring(0, inputLength);
    }
    ...
    private String alphaNumericOnly(String text) {
    ...
    private String buildPinyinInitials(String text) {
        StringBuilder sb = new StringBuilder();
        for (int i = 0; i < text.length(); i++) {
    ...
    /**
     * 根据用户输入判断是否为拼音首字母，若是则从片名缓存显示候选
     * 否则调用热词查询接口
     */
    private void updateCandidatesForPinyin(String input) {
        if (DynamicPinyinCandidateUtil.isPinyinInitials(input)) {
            // 如果输入是拼音首字母，直接在片名缓存中查找
            List<String> candidates = DynamicPinyinCandidateUtil.getCandidates(input);
            if (!candidates.isEmpty()) {
                wordAdapter.setNewData(candidates);
                return;
            }
        }
