...
public class HomeActivity extends BaseActivity {
    ...
    private void initData() {
        if (dataInitOk && jarInitOk) {
            ...
            LOG.i("movie-name crawl status: " + DynamicPinyinCandidateUtil.getCrawlStatusSummary());
            showLoading();
    ...
    public void refresh(RefreshEvent event) {
        if (event.type == RefreshEvent.TYPE_PUSH_URL) {
            String pushUrl = event.obj == null ? "" : String.valueOf(event.obj).trim();
            if (pushUrl.isEmpty()) {
                Toast.makeText(mContext, "推送地址为空", Toast.LENGTH_SHORT).show();
                return;
            }
            ...
            } else if (DefaultConfig.isVideoFormat(pushUrl)) {
                // Fallback for direct media urls when push_agent source is not configured.
                Intent newIntent = new Intent(mContext, PlayActivity.class);
                VodInfo vodInfo = createDirectPushVodInfo(pushUrl);
                newIntent.putExtra("VodInfo", vodInfo);
                newIntent.putExtra("sourceKey", vodInfo.sourceKey);
                newIntent.putExtra("directPush", true);
                newIntent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_CLEAR_TOP);
                HomeActivity.this.startActivity(newIntent);
            } else {
                Toast.makeText(mContext, "未配置push_agent，且推送地址不是直链视频", Toast.LENGTH_SHORT).show();
            }
        }
    }

    private VodInfo createDirectPushVodInfo(String pushUrl) {
        VodInfo vodInfo = new VodInfo();
        vodInfo.id = "push_" + System.currentTimeMillis();
        vodInfo.name = "网页推送";
        vodInfo.sourceKey = "push_direct";
        vodInfo.playFlag = "直链";
        vodInfo.playIndex = 0;
        vodInfo.seriesFlags = new ArrayList<>();
        vodInfo.seriesFlags.add(new VodInfo.VodSeriesFlag(vodInfo.playFlag));
        vodInfo.seriesMap = new LinkedHashMap<>();
        List<VodInfo.VodSeries> series = new ArrayList<>();
        series.add(new VodInfo.VodSeries("推送视频", pushUrl));
        vodInfo.seriesMap.put(vodInfo.playFlag, series);
        return vodInfo;
    }
