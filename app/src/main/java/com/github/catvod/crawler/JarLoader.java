        ...
        try {
            ...
            do {
                try {
                    ...
                    if (classInit != null) {
                        // Mark load success once Init class is discoverable. Some jars may fail in Init.init
                        // due to optional host-specific capabilities, but still work for normal spider calls.
                        success = true;
                        try {
                            Method method = classInit.getMethod("init", Context.class);
                            method.invoke(null, App.getInstance());
                        } catch (Throwable th) {
                            System.out.println("Init.init执行失败，继续尝试使用Spider: " + th.getMessage());
                            th.printStackTrace();
                        }
